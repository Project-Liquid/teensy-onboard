...
void PTE7300_I2C::CRC(bool tf) {
    _bUseCRC = tf;
}

unsigned int PTE7300_I2C::readRegister(
    uint8_t address, unsigned int number, uint16_t *buffer
) {
...
unsigned int PTE7300_I2C::readRegisterNoCRC(
    uint8_t address, unsigned int number, uint16_t *buffer
) {
    ...
    Wire.requestFrom(
        _nodeAddress, number * 2
    );  // Request register, note that register is 2 bytes wide
    ...
    if (bytesRead >= number * 2) {
        for (int i = 0; i < number; i++) {
            ...
            buffer[i] =
                highByte << 8 | lowByte;  // join two bytes into word (uint16)
        }
    }
...
unsigned int PTE7300_I2C::readRegisterCRC(
    uint8_t address, unsigned int number, uint16_t *buffer
) {
    unsigned int bytesRead = 0;
    ...
    Wire.beginTransmission(
        _nodeAddress | 1
    );  // indicate CRC-transmission by setting first address bit to 1
    Wire.write(address);  // Send register address
    Wire.write((((number * 2) - 1) << 4) | (crc4 & 0x0F));
    Wire.endTransmission();
    Wire.requestFrom(
        _nodeAddress | 1, (number * 2) + 1
    );  // Request registers, note that registers 2 bytes wide
    ...
    int crc8_received =
        Wire.read();  // read CRC byte, after reading the databuffer words
    ...
    if (crc8 != crc8_received) {
        // TODO: maybe actually check errors somehow
        // Serial.println("CRC ERROR!");
        for (int i = 0; i < number; i++) { buffer[i] = 0; }
        return 0;
    }
...
uint32_t PTE7300_I2C::readSERIAL() {
    ...
    return (((uint32_t)(serial[1]) << 16) & 0xFFFF0000) |
           ((uint32_t)(serial[0]) & 0x0000FFFF
           );  // join and type-cast to unsigned 32-bit integer
}
...
char PTE7300_I2C::calc_crc4(
    unsigned char polynom, unsigned char init, unsigned char *data,
    unsigned int len
) {
